...
if [ -f /usr/local/bin/cbsd ]; then
  ...
  sudo tmux split-window -h -p 50 -t 0 "cbsd jexec user=devel jname=frontend env HOST=${frontend_hostname} OFFLINE=${OFFLINE} BACKEND_URL=\"http://${backend_hostname}:5000\" /usr/src/bin/devel.sh"
  sudo tmux a -t "web"
else
