...
if [ "${REGGAE}" = "yes" ]; then
  ...
  sudo tmux split-window -h -p 66 -t 0 "make -C services/frontend BACKEND_URL=http://${backend_hostname}:5000 devel"
  sudo tmux split-window -h -p 50 -t 1 "make -C services/startkit BACKEND_URL=http://${backend_hostname}:5000 devel"
  if [ "${DESIGNER}" != "no" ]; then
